...
public sealed class WebService : IWebService
{
    ...
    public async Task<string> CheckSerbianMidRf()
    {
        ...
        var captchaSrc = htmlDocument.DocumentNode
            .SelectNodes("//img")
            .Where(x => x is not null)
            .Select(x => x.GetAttributeValue("src", ""))
            .FirstOrDefault(x => x.Contains("CodeImage", StringComparison.OrdinalIgnoreCase));

        ArgumentNullException.ThrowIfNull(captchaSrc);

        await Task.Delay(5000);

        //var captcha = await client.GetByteArrayAsync(client.BaseAddress + captchaSrc);

        var captcha = File.ReadAllBytes(Environment.CurrentDirectory + @"\CodeImage.jpeg");
